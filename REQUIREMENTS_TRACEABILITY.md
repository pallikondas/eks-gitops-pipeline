...
| 2.7a | ALB Ingress exposure | `helm/*/templates/ingress.yaml`; `ClusterIP` Services; shared `shopcore` ALB group | Implemented, AWS verification pending | Confirm an internet-facing ALB receives `/orders` and `/products` routes and has healthy targets. |
