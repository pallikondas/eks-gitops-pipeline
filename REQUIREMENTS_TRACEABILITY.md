...
**Release decision:** Ready for controlled AWS bootstrap, not yet production-validated
...
| 2.1e | Order persistent storage | `app/order-service/src/index.js`; `ORDER_TABLE_NAME`; DynamoDB Terraform resource | Implemented | Local fallback is used when no table is configured; live DynamoDB read/write test remains required after AWS bootstrap. |
...
| 2.2b | Images stored in ECR | `infrastructure/terraform/main.tf` ECR repositories; `.github/workflows/ci-cd.yaml`; `.github/workflows/catalog-service-ci-cd.yaml` | Implemented, AWS verification pending | Both services build, scan, and push immutable SHA tags after Terraform creates the repositories. |
| 2.3 | Kubernetes and Helm configuration managed in Git | `helm/`; `gitops/` | Implemented | Helm dev/prod templates render successfully. |
| 2.3a | GitOps controller | `gitops/apps/*.yaml` Argo CD Applications, including dev/prod application manifests | Implemented, AWS verification pending | Install Argo CD and sync Applications. Confirm Argo reports `Synced` and `Healthy`. |
...
| 2.4 | EKS data plane across multiple AZs | `infrastructure/terraform/main.tf`; `variables.tf` requires 3 AZs; managed node group uses private subnets | Implemented, AWS verification pending | Confirm nodes span at least two AZs with `kubectl get nodes -L topology.kubernetes.io/zone`. |
| 2.4a | HPA using resource metrics | `helm/order-service/templates/hpa.yaml`; `helm/catalog-service/templates/hpa.yaml`; `gitops/apps/metrics-server.yaml` | Implemented, AWS verification pending | Confirm Metrics Server is healthy and `kubectl get hpa -A` reports CPU/memory metrics. |
| 2.4b | Cluster Autoscaler, not Karpenter | `gitops/platform/cluster-autoscaler.yaml`; `gitops/apps/cluster-autoscaler.yaml`; Terraform ASG tags and IAM | Implemented, AWS verification pending | Confirm autoscaler pod is healthy, then run a pending-pod scale-out and scale-in test. |
...
| 2.5a | Order Service DynamoDB least-privilege access | `aws_iam_role.order_service`; `aws_iam_role_policy.order_service_dynamodb` | Implemented, AWS verification pending | Confirm the pod receives the role and can read/write only the orders table. |
...
| 2.6 | Prometheus metrics collection | `gitops/apps/observability-stack.yaml`; ServiceMonitors; `/metrics` endpoints | Implemented, AWS verification pending | Local `/metrics` endpoints verified. Confirm Prometheus targets are `UP` in EKS. |
| 2.6a | Grafana dashboards | `gitops/observability/grafana-shopcore-dashboard.yaml` | Implemented, AWS verification pending | Create `grafana-admin-credentials` through a secret manager, sync dashboard Application, and verify panels. |
| 2.6b | Application health, latency, autoscaling metrics | Node metrics middleware; dashboard panels for request rate, p95 latency, ready pods, HPA replicas | Implemented, AWS verification pending | Generate traffic and confirm non-empty Prometheus series and Grafana panels. |
| 2.7 | AWS Load Balancer Controller | Terraform IAM policy/role; `gitops/platform/aws-load-balancer-controller-serviceaccount.yaml`; `infrastructure/README.md` Helm install | Implemented, AWS verification pending | Install the controller chart and confirm controller pods are healthy. |
| 2.7a | ALB Ingress exposure | `helm/*/templates/ingress.yaml`; `ClusterIP` Services; shared `shopcore` ALB group; deployment workflow waits for the ALB controller | Implemented, AWS verification pending | Confirm an internet-facing ALB receives `/orders` and `/products` routes and has healthy targets after both service image promotions. |
| 2.7b | External access, routing, failover | `README.md` Host-header-aware ALB curl checks, pod deletion, and ALB validation procedures | Partial | Execute the direct ALB checks before DNS, then record DNS, HTTP responses, target health, and pod replacement evidence. |
...
Until this post-bootstrap evidence is collected, the implementation is **deployment-ready but not production-verified**.
