...
| 2.2b | Images stored in ECR | `infrastructure/terraform/main.tf` ECR repositories; GitHub Actions workflow | Implemented, AWS verification pending | Push immutable image tags after Terraform creates the repositories. |
| 2.3 | Kubernetes and Helm configuration managed in Git | `helm/`; `gitops/` | Implemented | Helm dev/prod templates render successfully. |
| 2.3a | GitOps controller | `gitops/apps/*.yaml` Argo CD Applications | Implemented, AWS verification pending | Install Argo CD and sync Applications. Confirm Argo reports `Synced` and `Healthy`. |
